...
///
/// `Create` (`0x02`) is the one to stay away from on a read path — it *deletes the current property
/// value* and opens an empty stream, so a mistyped flag on a body read would destroy the body.
///
/// [MS-OXCPRPT] §2.2.14.1 — `OpenModeFlags`
const STREAM_READ_ONLY: u8 = 0x00;
...
pub(crate) fn encode_open_stream(w: &mut Writer, input: u8, output: u8, tag: PropertyTag) {
...
mod tests {
    ...
    fn the_stream_requests_match_the_spec_layouts() {
        ...
        encode_open_stream(&mut w, 1, 2, PropertyTag::BODY);
        assert_eq!(
            w.finish(),
